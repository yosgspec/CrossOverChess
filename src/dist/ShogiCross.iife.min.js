...
┠${Array(t).fill("──").join("┼")}┨
`),s+this.field.map(t=>r+t.map(t=>t.piece?.toString(a)??t.toString(e)).join(o)+r).join(n)+i+this.stand.toString(e)}async downloadImage(e,a,t){await g(this.canvas,e??this.name??"shogicross",a,t)}}class es{static buttonTexts="<>\uD83D\uDD04\uD83D\uDD01\uD83D\uDCF7";constructor(e,t){this.board=e;let s=new Map([["undoRecord",{title:"一手戻る",text:"&lt;&lt;",onclick:()=>e.undoRecord()}],["redoRecord",{title:"一手進む",text:"&gt;&gt;",onclick:()=>e.redoRecord()}],["rotateLeft",{title:"盤面を左回転",text:"\uD83D\uDD04",onclick:()=>e.rotate(!1)}],["rotateRight",{title:"盤面を右回転",text:"\uD83D\uDD01",onclick:()=>e.rotate()}],["downloadImage",{title:"画像を保存",text:"\uD83D\uDCF7",onclick:()=>e.downloadImage()}]]);t??=[...s.keys(),"textRecord"];let i=Date.now().toString();for(let[r,{onclick:o}]of(this.component=document.createElement("div"),this.component.id=i,this.component.style.display="flex",this.component.style.maxWidth=e.canvas.width+"px",this.component.innerHTML=`${[...s].filter(([e])=>t.includes(e)).map(([e,{title:t,text:s}])=>`<button id="${e}${i}" title="${t}" style="font-family:${a.names};">${s}</button>`).join("")}${t.includes("textRecord")?`<select id="textRecord${i}" style="flex-grow:1; font-family:${a.names};"><option></option></select>`:""}`,s))t.includes(r)&&(this.component.querySelector(`#${r}${i}`).onclick=o);if(!t.includes("textRecord"))return;let n=e.onDrawed??(()=>{});e.onDrawed=async a=>{setTimeout(()=>{let t=this.component.querySelector(`#textRecord${i}`),s=t.querySelector("option"),r=t.cloneNode(!1);a.record.forEach((t,i)=>{let o=s.cloneNode(!1);o.textContent=e.record2String(t,i),i===a.turn&&(o.selected=!0),r.appendChild(o)}),r.onchange=a=>e.moveRecord(a.target.selectedIndex),t.replaceWith(r)}),n(a)}}add(){let{canvas:e}=this.board;e.after(this.component)}remove(){this.component.remove()}}let ei=()=>[...new Set([...es.buttonTexts+Object.values(r).map(({displayText:e})=>e).join("")+Object.values(o).map(({display:e})=>e?e.join(""):"").join("")])].sort().join("");return Object.assign(a,{imported:!1,names:"",async importAsync(){if(this.imported)return;let e=ei(),t=Date.now().toString();return this.names=a.fonts.map(e=>`"${e[0]}${t}"`).join(",")+",serif",Promise.all(a.fonts.map(async([a,s])=>{let i=a.replace(/ /g,"+"),r=`https://fonts.googleapis.com/css2?family=${i}:wght@${s}&text=${e}`,o=await fetch(r);if(!o.ok)return;let n=(await o.text()).match(/url\(.+?\)/g);if(!n)throw Error("Not found font.");for(let l of n){let d=new FontFace(`${a}${t}`,l);document.fonts.add(d),await d.load().catch(()=>{})}})).then(e=>this.imported=!0)}}),e.Board=R,e.Piece=h,e.boards=i,e.canvasFont=a,e.canvasImage=m,e.gameSoft=t,e.games=s,Object.defineProperty(e,Symbol.toStringTag,{value:"Module"}),e}({});