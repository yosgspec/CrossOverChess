...
┠${Array(t).fill("──").join("┼")}┨
`),s+this.field.map(t=>r+t.map(t=>t.piece?.toString(a)??t.toString(e)).join(n)+r).join(o)+i+this.stand.toString(e)}async downloadImage(e,a){await h(this.canvas,e,a)}}return e.Board=j,e.Piece=B,e.boards=i,e.canvasFont=a,e.canvasImage=c,e.gameSoft=t,e.games=s,Object.defineProperty(e,Symbol.toStringTag,{value:"Module"}),e}({});