...
┠${Array(t).fill("──").join("┼")}┨
`),s+this.field.map(t=>r+t.map(t=>t.piece?.toString(a)??t.toString(e)).join(o)+r).join(n)+i+this.stand.toString(e)}async downloadImage(e,a,t){await oe(this.canvas,e??this.name??"shogicross",a,t)}}class te{static buttonTexts="<>\uD83D\uDD04\uD83D\uDD01\uD83D\uDCF7";constructor(e,a){this.board=e;let t=new Map([["undoRecord",{title:"一手戻る",text:"&lt;&lt;",onclick:()=>e.undoRecord()}],["redoRecord",{title:"一手進む",text:"&gt;&gt;",onclick:()=>e.redoRecord()}],["rotateLeft",{title:"盤面を左回転",text:"\uD83D\uDD04",onclick:()=>e.rotate(!1)}],["rotateRight",{title:"盤面を右回転",text:"\uD83D\uDD01",onclick:()=>e.rotate()}],["downloadImage",{title:"画像を保存",text:"\uD83D\uDCF7",onclick:()=>e.downloadImage()}]]);a??=[...t.keys(),"textRecord"];let s=Date.now().toString();for(let[i,{onclick:r}]of(this.component=document.createElement("div"),this.component.id=s,this.component.style.display="flex",this.component.style.maxWidth=e.canvas.width+"px",this.component.innerHTML=`${[...t].filter(([e])=>a.includes(e)).map(([e,{title:a,text:t}])=>`<button id="${e}${s}" title="${a}" style="font-family:${O.names};">${t}</button>`).join("")}${a.includes("textRecord")?`<select id="textRecord${s}" style="flex-grow:1; font-family:${O.names};"><option></option></select>`:""}`,t))a.includes(i)&&(this.component.querySelector(`#${i}${s}`).onclick=r);if(!a.includes("textRecord"))return;let o=e.onDrawed??(()=>{});e.onDrawed=async a=>{setTimeout(()=>{let t=this.component.querySelector(`#textRecord${s}`),i=t.querySelector("option"),r=t.cloneNode(!1);a.record.forEach((t,s)=>{let o=i.cloneNode(!1);o.textContent=e.record2String(t,s),s===a.turn&&(o.selected=!0),r.appendChild(o)}),r.onchange=a=>e.moveRecord(a.target.selectedIndex),t.replaceWith(r)}),o(a)}}add(){let{canvas:e}=this.board;e.after(this.component)}remove(){this.component.remove()}}let fe=()=>[...new Set([...te.buttonTexts+Object.values(H).map(({displayText:e})=>e).join("")+Object.values(J).map(({display:e})=>e?e.join(""):"").join("")])].sort().join("");Object.assign(O,{imported:!1,names:"",async importAsync(){if(this.imported)return;let e=fe(),a=Date.now().toString();return this.names=O.fonts.map(e=>`"${e[0]}${a}"`).join(",")+",serif",Promise.all(O.fonts.map(async([t,s])=>{let i=t.replace(/ /g,"+"),r=`https://fonts.googleapis.com/css2?family=${i}:wght@${s}&text=${e}`,o=await fetch(r);if(!o.ok)return;let n=(await o.text()).match(/url\(.+?\)/g);if(!n)throw Error("Not found font.");for(let l of n){let d=new FontFace(`${t}${a}`,l);document.fonts.add(d),await d.load().catch(()=>{})}})).then(e=>this.imported=!0)}});export{ee as Board,A as Piece,U as boards,O as canvasFont,G as canvasImage,ae as gameSoft,Y as games};