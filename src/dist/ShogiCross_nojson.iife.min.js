...
${n}持駒：${n}`);return i+r}}let C=Object.keys(g.degChars),_=()=>({panel:null,piece:null});class w{static #a=new Map([[0," "],[90,">"],[180,"v"],[270,"<"]]);static #b=new Map([...w.#a].map(([e,a])=>[e,RegExp(a,"g")]));static #c=new Map([...w.#a].map(([e,a])=>[a,e]));static #d=new Map([[0,"先手の持駒"],[90,"次手の持駒"],[180,"後手の持駒"],[270,"四手の持駒"]]);static #e=new Map([...w.#d].map(([e,a])=>[a,e]));static #f=["","一","二","三","四","五","六","七","八","九"];static #g=["","十","二十","三十","四十","五十","六十","七十","八十","九十"];static #h(k,x=!0){return!x&&k<=1?"":w.#g[0|k/10]+w.#f[k%10]}static #i(N,E=!0){if(E&&""===N)return 1;if(!isNaN(N))return 0|N;let b=w.#g.findIndex(e=>""!==e&&RegExp("^"+e).test(N));b<0&&(b=0);let X=w.#f.findIndex(e=>""!==e&&RegExp(e+"$").test(N));return X<0&&(X=0),10*b+X}static #j(v){return 10<=v?v:"０１２３４５６７８９"[v%10]}static #k=" ・";static #l(P){return P?w.#a.get(P.deg)+P.char:w.#k}static #m(O,L=0){let M=new Map;return O.stocks.get(L).forEach(({char:e})=>{M.has(e)||M.set(e,0),M.set(e,M.get(e)+1)}),w.#d.get(L)+"："+[...M].map(([e,a])=>e+w.#h(a,!1)).join(" ")}static convTextPieces(e){let a=[],t=[];e.split(/\r|\n|\r\n/).forEach(e=>{[...w.#e.keys()].some(a=>RegExp(`^${a}`).test(e))?t.push(e):a.push(e.slice(1))});let s=a.slice(2,-1).join(`
`);w.#b.forEach((e,a)=>{s=s.replace(e,g.degChars[a])});let i=t.flatMap(e=>{let[a,t]=e.split(/：/);if(""===t)return"";let s=w.#e.get(a),i=g.degChars[s];return t.split(/\s/).map(e=>{let a=e[0],t=e.slice(1);return(i+a).repeat(w.#i(t))})}).join("");return`${s}
